...
const dexStreams: DexStreamsState = {
    ws: null,
    state: 'disconnected',
    markets: {},
    assetMaps: {},
...
export function startDexStream(
    exchangeId: ExchangeId,
    exchange: CcxtExchange,
    isLinearSwap: IsLinearSwapFn,
    ...
    dexStreams.state = StreamState.CONNECTING;
...
function buildDexMarketMaps(exchange: CcxtExchange, isLinearSwap: IsLinearSwapFn): void {
    dexStreams.markets = {};
...
function connectDexStream(wsUrl: string): void {
    safeClose(dexStreams.ws);
    ...
    ws.onopen = () => {
        dexStreams.reconnectAttempt = 0;
        dexStreams.state = StreamState.CONNECTED;
        safeSend(ws, {
            method: 'subscribe',
    ...
    ws.onerror = () => {
        console.error('hyperliquid dex error: connection error');
    };
}

function handleDexAllMids(data: DexAllMidsData): void {
    const dex = data?.dex;
    if (!dex || dex === '') return;
...
function handleDexAssetCtxs(data: DexAssetCtxsData): void {
...
function startDexPing(): void {
    stopDexPing();
    const config = EXCHANGE_CONFIG.hyperliquid;
    dexStreams.pingInterval = setInterval(() => {
        safeSend(dexStreams.ws, { method: 'ping' });
    }, config.pingInterval!);
}

function stopDexPing(): void {
...
function scheduleDexReconnect(wsUrl: string): void {
    if (dexStreams.reconnectTimeout) return;

    const delay = calculateBackoff(dexStreams.reconnectAttempt++);
    dexStreams.state = StreamState.RECONNECTING;
...
function flushDexBatch(): void {
    ...
    for (const raw of dexStreams.messageBuffer) {
        try {
            ...
            console.error('hyperliquid dex parse error:', (e as Error).message);
        }
    }
    ...
    const pooled = getPooledUpdates(config.poolKeys!.dex, size);
    let idx = 0;
...
export function stopDexStream(exchangeId: ExchangeId): void {
    if (exchangeId !== 'hyperliquid') return;

    dexStreams.state = StreamState.DISCONNECTED;
...
export function isDexStreamActive(): boolean {
